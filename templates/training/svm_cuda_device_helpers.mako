...
__device__ void coopExtractRowVector(float* data, int dataPitch, int index, int dimension, float* destination) {
  ...
  for(int currentDim = threadIdx.x; currentDim < min(1500,dimension); currentDim += blockDim.x) {
...
// ==================== REDUCE OPERATORS ===============
inline __device__ void argMax(int mapAIndex, float mapAValue, int mapBIndex, float mapBValue, int* reduceIndex, float* reduceValue) {
...
inline __device__ void argMin(int mapAIndex, float mapAValue, int mapBIndex, float mapBValue, int* reduceIndex, float* reduceValue) {
	if (mapBValue < mapAValue) {
		*reduceIndex = mapBIndex;
		*reduceValue = mapBValue;
	} else {
