  ...
  float lowKernel = 0;
    ...
    if (iHighCompute && iLowCompute) {
      ...
      lowKernel = Kernel::kernel(devData + globalIndex, devDataPitchInFloats, devData + globalIndex + (devDataPitchInFloats * nDimension), xILow, 1, parameterA, parameterB, parameterC);
		...
		{
    if (iHighCompute && iLowCompute) {
      Kernel::dualKernel(devData + globalIndex, devDataPitchInFloats, devData + globalIndex + (devDataPitchInFloats * nDimension), devTransposedData + iHigh*devTransposedDataPitchInFloats, 1, devTransposedData + iLow*devTransposedDataPitchInFloats, 1, parameterA, parameterB, parameterC, highKernel, lowKernel);
    } else if (iHighCompute) {
      highKernel = Kernel::kernel(devData + globalIndex, devDataPitchInFloats, devData + globalIndex + (devDataPitchInFloats * nDimension), devTransposedData + iHigh*devTransposedDataPitchInFloats, 1, parameterA, parameterB, parameterC);
    } else if (iLowCompute) {
      lowKernel = Kernel::kernel(devData + globalIndex, devDataPitchInFloats, devData + globalIndex + (devDataPitchInFloats * nDimension), devTransposedData + iLow*devTransposedDataPitchInFloats, 1, parameterA, parameterB, parameterC);
    }
		}
