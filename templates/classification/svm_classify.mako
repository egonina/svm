              ...
              float b)
{
	...
	{
		printf("Error: Unknown kernel type - %d\n", kernel_type);
		exit(0);
	}
	...
	for(int dataoffset=0; dataoffset<total_nPoints; dataoffset += nPoints) 
	{
		...
		dim3 blocksDots(intDivideRoundUp(nSV, ${num_threads}), intDivideRoundUp(nPoints, ${num_threads}));
		...
		if(kernel_type == 1)
		{
			launchComputeKernelsReduce(devDots, devDotsPitchInFloats,
                                       ...
                                       devLocalValue, 1<<int(ceil(log2((float)${num_threads}))-1),
